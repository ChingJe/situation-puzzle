...
- 只從 truth 抽取。
- 覆蓋核心因果鏈。
- 讓每條 fact 都能用於問答或解答判定。

不得做：

- 不得新增 truth 沒有的設定。
- 不得抽取氣氛、情緒、文學化描述。
- 不得把 forbidden assumption 寫成 key fact。
...
若 key facts 遺漏、過多、過少或新增設定，打回 `extract_key_facts`。

若 truth 本身不清楚導致 key facts 無法抽取，打回 `expand_truth`。
...
- 使用 truth/key_facts 已存在的角色、物品、行動與結果。

不得做：

- 不得解釋原因。
- 不得寫真正原因、動機、關鍵行動者的內心。
- 不得新增 truth/key_facts 沒有的線索。
- 不得加入第二個異常。
...
若 forbidden assumptions 條數錯誤或否定主題/謎面事實，打回 `generate_forbidden_assumptions`。
...
- surface story 的每個異常都能在 truth/key_facts 找到根據。
- surface story 沒有新增 truth/key_facts 沒有的線索。
- key facts 都來自 truth。
...
- `extract_key_facts`
  - key facts 遺漏核心因果。
  - key facts 新增 truth 沒有的設定。
  - key facts 條數不符合設定。

- `write_surface_story`
  - 謎面格式不符。
  - 謎面洩漏原因。
  - 謎面新增 truth/key_facts 沒有的線索。
  - 謎面只有情緒或氣氛，沒有客觀異常。
...
- 驗收重點：是否產生可玩的核心因果與可判定 key facts，而不是單次生成速度。
