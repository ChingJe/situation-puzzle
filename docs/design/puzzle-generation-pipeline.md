...
後續實測又顯示，題目可能通過一致性檢查但仍過於直覺，且 `judge_solution` 會因 `key_facts` 過細而對玩家短解答過嚴。因此目前流程採用 `PuzzleV2` 品質 contract，將解答必要事實與支撐事實拆分。詳細設計見 `docs/design/puzzle-quality-contract.md`。

## 設計目標

- 讓每個 LLM 節點只負責一件事，降低單次生成負擔。
- 先固定可解的核心因果鏈，再產生玩家可見謎面。
- `required_solution_facts` 與 `supporting_facts` 從真相抽取，不讓模型自由另創。
...
目前正式方向已將 `extract_key_facts` 調整為 `extract_solution_facts`，輸出 `required_solution_facts` 與 `supporting_facts`。為了相容既有程式，finalize 階段可合併產生 legacy `key_facts`。
...
truth: str | None
required_solution_facts: list[RequiredSolutionFact]
supporting_facts: list[SupportingFact]
surface_story: str | None
misleading_assumptions: list[str]
...
- 將通關必要事實與支撐問答的背景事實分開。
- 不創作新設定。
- 每條 required fact 必須是玩家合理正解需要命中的具體事實。
- supporting facts 可支撐問答或完整揭示，但不應讓玩家提交解答時逐字命中。
...
- 不寫「其實不成立」的推論。
- 不解釋原因，不透露關鍵行動者的動機。
- 必須為 required facts 中的真正原因提供至少一個玩家可觀察的公平線索。
...
- 不得直接說出真正原因或完整手法，但可用痕跡、位置變化、時間關聯、物件狀態、異常聲音或旁人反應作為公平線索。
...
- `surface_story` 有為真正原因留下可觀察入口。
- `required_solution_facts` 與 `supporting_facts` 都能在 `truth` 找到根據。
...
target_node=generate_core_truth
  -> 重新生成 core_truth、truth、required/supporting facts、surface_story、assumptions

target_node=expand_truth
  -> 保留 topic_interpretation/core_truth，重寫 truth 後重抽 required/supporting facts 與謎面

target_node=extract_solution_facts
  -> 保留 truth，重抽 required/supporting facts，重跑 reviewer

target_node=write_surface_story
  -> 保留 truth 與 required/supporting facts，只重寫 surface_story，再重跑 reviewer

target_node=generate_assumptions
  -> 只重寫 misleading/forbidden assumptions，再重跑 reviewer
```
...
- deterministic gate 可阻擋空 required solution facts。
...
- 真相能一句話概括。
- 謎面不否定真相，也不被真相否定。
- required solution facts 是判定解答用的必要因果鏈。
